...
export default pubsub => ({
    Query: {
        // users: withAuth(['user:view:all'], (obj, { orderBy, filter }, { UsersApi }) => {
        //     return {usersQuery: true}; //User.getUsers(orderBy, filter);
        // }),
        user(obj, { userId }, { user, UsersApi, req }) {
            return { user: UsersApi(req).getUser(userId) };

            // const e = new FieldError();
            // e.setError('user', t('user:accessDenied'));
            // return { user: null, errors: e.getErrors() };
        },
        currentUser(obj, args, { UsersApi, user }) {
            return user || null;
        }
    },
    // User: {
    ...
    // addUser: withAuth(
    //     (obj, args, { User, user }) => {
    //         return user.uid !== args.input.id ? ['user:create'] : ['user:create:self'];
    //     },
    ...
    // editUser: withAuth(
    //     (obj, args, { User, user }) => {
    //         return user.uid !== args.input.id ? ['user:update'] : ['user:update:self'];
    //     },
    //     async (obj, { input }, { User, user, req: { t } }) => {
    //         const isAdmin = () => user.role === 'admin';
    //         const isSelf = () => user.uid === input.id;
    //         try {
    //             const e = new FieldError();
    //             const userExists = await User.getUserByUsername(input.username);
    //
    ...
    // deleteUser: withAuth(
    //     (obj, args, { User, user }) => {
    //         return user.uid !== args.id ? ['user:delete'] : ['user:delete:self'];
    //     },
    ...
    //         const isSelf = () => context.user.uid === id;
    //
    //         try {
    //             const e = new FieldError();
